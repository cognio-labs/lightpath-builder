...
export async function POST(request: NextRequest) {
  try {
    ...
    const primaryModel = process.env.OPENROUTER_CHAT_MODEL || "nvidia/nemotron-3-super-120b-a12b:free";
      ...
      for (const model of modelsToTry) {
        try {
          ...
          if (llmRes.ok) {
            ...
            console.warn(`Model ${model} returned error status:`, llmRes.status);
          }
        } catch (modelErr) {
...
function synthesizeKnowledgeResponse(
  ...
  if (q.includes("guru ji kaun") || q.includes("sakshi shree kaun") || q.includes("who is sakshi shree") || q.includes("who is guru")) {
    if (isHindi) {
      return "🙏 Sakshi Shree Science Divine Movement ke margdarshak aur enlightened spiritual master hain. Unka sandesh hai 'Bheetar se sanyaas, bahar se sansaar' — yaani sansaar ke kartavyon ko nibhate hue bheetari shanti aur sakshi bhaav mein jeena.";
    }
    return "🙏 Sakshi Shree is an enlightened spiritual master and the guide behind the Science Divine Movement. He teaches 'Bheetar se sanyaas, bahar se sansaar' — cultivating inner peace while fulfilling worldly duties with excellence.";
  }
