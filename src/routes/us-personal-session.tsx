...
export const Route = createFileRoute("/us-personal-session")({
  head: () => ({
    meta: [
      { title: "US Personal Session | Science Divine Foundation" },
      { name: "description", content: "Schedule a US-based personal guidance session with Sakshi Shree. Direct spiritual guidance and reading." },
    ],
  }),
  component: Page,
});
...
const FAQS = [
  {
    ...
    a: "US personal sessions are conducted online via high-quality video call. Our team will coordinate the time zone details with you.",
  },
  {
    q: "How is the session scheduled?",
    a: "Our team contacts you within 24 hours after booking to arrange a time slot suitable for your US time zone.",
  },
...
function Page() {
  return (
    <>
      <PageHero
        eyebrow="US Personal Session"
        title="US-Sadhak Personal Guidance"
        subtitle="Sakshi Shree reads your energy like an open book. Request a dedicated US personal session online."
      >
        <a href="#book" className="btn-gold rounded-full px-8 py-3.5 text-sm font-semibold">
          Reserve Your US Session
        </a>
      </PageHero>
      ...
      <section className="section-pad bg-white">
        <div className="container-page grid md:grid-cols-3 gap-6">
          {[
            {
              icon: Sparkles,
              title: "US Time Zone Coordination",
              desc: "Dedicated scheduling tailored to US Eastern, Central, Mountain, and Pacific time zones.",
            },
            {
              icon: Compass,
            ...
            {[
              {
                ...
                desc: "Donation to Jhuggi Jhopdi Shiksha Sewa Mission (80G tax-exempt).",
              },
              {
                n: 2,
                title: "Time Zone Coordination",
                desc: "Our team coordinates timezone details to find a perfect online slot.",
              },
              {
                n: 3,
                title: "US Session",
                desc: "Meet Sakshi Shree online. Receive your reading. Return home changed.",
              },
            ].map((s) => (
    ...
    try {
      await submit({
        data: {
          ...
          message: "US Callback request from Book Session page",
        },
      });
