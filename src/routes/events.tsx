...
export const Route = createFileRoute("/events")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Upcoming events, retreats, and gatherings with Sakshi Shree. Meditation shivirs, satsangs, and mahotsavs." },
...
function Page() {
  return (
    <>
      <PageHero eyebrow="Gather Together" title="Find the events for conscious awakening" subtitle="From intimate satsangs to mahotsavs — join the sangha in person or online." />
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {EVENTS.map((e, i) => (
              <div key={i} className="glass-card rounded-2xl p-6 hover-lift">
                <div className="inline-block px-3 py-1 rounded-full bg-primary/10 text-primary text-[10px] font-bold uppercase tracking-widest mb-4">{e.status}</div>
                ...
                <button className="btn-gradient rounded-full px-5 py-2 text-sm font-semibold">Register</button>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {GALLERY.map(src => (
              <div key={src} className="aspect-[4/3] rounded-2xl overflow-hidden hover-lift">
                <img src={src} alt="Event moment" loading="lazy" className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
              </div>
            ))}
          </div>
        </div>
