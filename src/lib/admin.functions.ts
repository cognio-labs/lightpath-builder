...
export const listCoursesPublic = createServerFn({ method: "GET" }).handler(async () => {
  const { data, error } = await publicClient().from("courses").select("*").eq("published", true).order("sort_order");
...
export const getCoursePublic = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data: slug }) => {
    const { data } = await publicClient().from("courses").select("*").eq("slug", slug).maybeSingle();
    return data;
  });

export const listEventsPublic = createServerFn({ method: "GET" }).handler(async () => {
  const { data, error } = await publicClient().from("events").select("*").eq("published", true).order("starts_at", { ascending: true, nullsFirst: false });
  if (error) throw new Error(error.message);
  return data ?? [];
});

export const listTestimonialsPublic = createServerFn({ method: "GET" }).handler(async () => {
  const { data, error } = await publicClient().from("testimonials").select("*").eq("published", true).order("sort_order");
  if (error) throw new Error(error.message);
  return data ?? [];
});
...
export const getSolutionPublic = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data: slug }) => {
    ...
    const [page, articles] = await Promise.all([
      ...
      c.from("solution_articles").select("*").eq("solution_slug", slug).eq("published", true).order("sort_order"),
    ]);
...
export const adminListSubmissions = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await context.supabase.from("form_submissions").select("*").order("created_at", { ascending: false });
    if (error) throw new Error(error.message);
    return data ?? [];
  });
...
export const adminUpdateSubmission = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { id: string; status?: "new" | "contacted" | "closed"; admin_notes?: string }) => d)
...
// Generic admin CRUD
const TABLES = ["courses", "events", "testimonials", "solution_pages", "solution_articles"] as const;
...
export const adminList = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data: table, context }) => {
    await assertAdmin(context.supabase, context.userId);
    const orderCol = table === "events" ? "starts_at" : table === "solution_pages" ? "slug" : "sort_order";
    const { data, error } = await context.supabase.from(table).select("*").order(orderCol as any, { ascending: true, nullsFirst: false });
    if (error) throw new Error(error.message);
    return data ?? [];
  });
...
export const adminUpsert = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { table: AdminTable; row: Record<string, any> }) => ({
