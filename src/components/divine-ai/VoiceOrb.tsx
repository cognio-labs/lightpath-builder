...
import { Sparkles, Mic, Volume2, Loader2, AlertCircle, Radio, AudioLines } from "lucide-react";
...
export function VoiceOrb({ state, audioLevel = 0, onClick, size = "md" }: VoiceOrbProps) {
  ...
  const stateConfig = {
    ...
    requesting_permission: {
      ...
      ringColor: "border-sky-400 animate-spin",
      innerShadow: "0 0 45px rgba(14, 165, 233, 0.6)",
      icon: <Radio className="w-8 h-8 text-white animate-pulse" />,
      pulseScale: 1.04,
    },
    listening: {
      orbBg: "from-sky-400 via-cyan-400 to-blue-600",
      glowBg: "rgba(56, 189, 248, 0.75)",
      ringColor: "border-sky-300 shadow-sky-400/50",
      innerShadow: "0 0 50px rgba(56, 189, 248, 0.8)",
    ...
    recording: {
      ...
      ringColor: "border-rose-400 animate-ping",
      innerShadow: "0 0 50px rgba(244, 63, 94, 0.75)",
    ...
    transcribing: {
      ...
      ringColor: "border-amber-400/80 animate-spin",
      innerShadow: "0 0 45px rgba(251, 191, 36, 0.6)",
    ...
    thinking: {
      ...
      ringColor: "border-purple-300 animate-spin",
      innerShadow: "0 0 50px rgba(168, 85, 247, 0.7)",
    ...
    speaking: {
      ...
      ringColor: "border-yellow-300 animate-pulse",
      innerShadow: "0 0 60px rgba(250, 204, 21, 0.9)",
    ...
    interrupted: {
      ...
      innerShadow: "0 0 40px rgba(56, 189, 248, 0.6)",
  ...
  return (
    ...
    >
      {/* 🌟 1. SACRED ROTATING MANDALA BACKGROUND AURA */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-40 group-hover:opacity-60 transition-opacity">
        <svg
          viewBox="0 0 200 200"
          className="w-56 h-56 md:w-64 md:h-64 animate-spin-slow text-amber-500/30 dark:text-amber-300/20"
        ...
        >
          ...
          <circle cx="100" cy="100" r="60" fill="none" stroke="currentColor" strokeWidth="0.8" strokeDasharray="3 3" />
          {/* Sacred 12 Petal Geometry */}
      ...
      <div
        ...
        style={{
          ...
          transform: `scale(${stateConfig.pulseScale * 1.35})`,
        }}
      />

      {/* 🌟 3. PULSING RIPPLE RINGS */}
      <div
        ...
        style={{
          ...
          opacity: state === "speaking" || state === "recording" || state === "listening" ? 0.85 : 0.35,
        }}
      />
      <div
        className={`absolute inset-0 rounded-full border transition-all duration-500 ${stateConfig.ringColor}`}
        style={{
          transform: `scale(${stateConfig.pulseScale * 1.48})`,
          opacity: state === "speaking" || state === "recording" || state === "listening" ? 0.5 : 0.15,
        }}
      />

      {/* 🌟 4. MAIN 3D CRYSTAL SACRED ORB */}
      <div
        ...
        style={{
          ...
          boxShadow: `${stateConfig.innerShadow}, inset 0 3px 18px rgba(255,255,255,0.85), inset 0 -6px 16px rgba(0,0,0,0.35)`,
        }}
      >
        {/* 🌟 3D Glass Specular Reflection (Top Left Shimmer) */}
        <div
          className="absolute top-2 left-3 w-2/5 h-2/5 rounded-full pointer-events-none"
          style={{
        ...
        <div
          className="absolute bottom-1 right-2 w-1/3 h-1/4 rounded-full pointer-events-none opacity-40 blur-[2px]"
          style={{
