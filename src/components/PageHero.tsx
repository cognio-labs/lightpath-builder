  ...
  return (
    <section className="relative overflow-hidden ethereal-bg text-white">
      ...
      <div className="container-page relative py-20 md:py-28 text-center max-w-4xl mx-auto">
        {eyebrow && (
          <div className="inline-block px-4 py-1.5 rounded-full glass-dark text-xs font-semibold uppercase tracking-wider mb-5 animate-fade-in">
            {eyebrow}
          </div>
        )}
        <h1 className="font-display text-4xl md:text-6xl font-bold leading-[1.1] animate-fade-in">
          {title}
        </h1>
        {subtitle && (
          <p className="mt-6 text-base md:text-xl text-white/85 max-w-2xl mx-auto animate-fade-in">
            {subtitle}
          </p>
        )}
        {children && (
...
export function SectionHeading({
  ...
  return (
    <div className={`mb-10 md:mb-14 ${center ? "text-center max-w-3xl mx-auto" : ""}`}>
      {eyebrow && (
        <div className="text-xs font-bold uppercase tracking-widest gradient-text mb-3">
          {eyebrow}
        </div>
      )}
