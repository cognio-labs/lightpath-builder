export function PageHero({
    ...
    return (
      <section className="relative overflow-hidden bg-white">
        ...
        <div className="container-page relative py-20 md:py-28">
          <div className={`grid md:grid-cols-2 gap-10 items-center ${imageSide === "left" ? "md:grid-flow-row" : ""}`}>
            <div className={`space-y-6 ${imageSide === "left" ? "md:order-2" : "md:order-1"}`}>
              {eyebrow && (
              ...
              <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 leading-[1.1]">
                {title}
              </h1>
              {subtitle && (
                <p className="text-base md:text-lg text-gray-500 leading-relaxed max-w-xl">{subtitle}</p>
              )}
              {children && (
            ...
            <div className={`${imageSide === "left" ? "md:order-1" : "md:order-2"} flex justify-center`}>
              <img
                ...
                style={{ mixBlendMode: "multiply", maxHeight: "520px" }}
              />
            </div>
          </div>
        </div>
  ...
  return (
    <section className="relative overflow-hidden" style={{ background: "linear-gradient(135deg, #FFFBF0 0%, #FFF8E7 60%, #FFFFFF 100%)" }}>
      <div
      ...
      <div className="container-page relative py-20 md:py-28 text-center max-w-4xl mx-auto">
        ...
        <h1 className="font-display text-4xl md:text-6xl font-bold text-gray-900 leading-[1.1] animate-fade-in">
          {title}
        </h1>
        {subtitle && (
          <p className="mt-6 text-base md:text-xl text-gray-500 max-w-2xl mx-auto leading-relaxed animate-fade-in">
            {subtitle}
          </p>
        )}
        {children && (
          <div className="mt-8 flex flex-wrap justify-center gap-3 animate-fade-in">{children}</div>
        )}
      </div>
    </section>
  );
...
export function SectionHeading({
  ...
  return (
    <div className={`mb-10 md:mb-14 ${center ? "text-center max-w-3xl mx-auto" : ""}`}>
      ...
      {subtitle && <p className="mt-4 text-base md:text-lg text-gray-500 leading-relaxed">{subtitle}</p>}
    </div>
  );
}
