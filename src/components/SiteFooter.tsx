      ...
      <div className="container-page py-16 grid gap-10 md:grid-cols-2 lg:grid-cols-4">
        <div>
          ...
          <p className="text-sm text-white/80 leading-relaxed">
            Science Divine Foundation — a global movement for Sound Body, Sound Mind, and Self
            Realization, guided by enlightened master Sakshi Shree.
          </p>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-white/80">
            ...
            <li>
              <Link to="/har-ghar-shiksha" className="hover:text-white">
                Join Volunteer
              </Link>
            </li>
            <li>
              <Link to="/book-session" className="hover:text-white">
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-white/80">
            {COURSES.map((c) => (
              <li key={c.slug}>
                <Link to={`/${c.slug}` as string} className="hover:text-white">
                  {c.title}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm text-white/80">
            <li className="flex gap-2">
              ...
              <span>8, Avantika Rd, Chiranjiv Vihar, Ghaziabad, UP 201001</span>
            </li>
            <li className="flex gap-2">
          ...
          <form className="mt-5 flex gap-2" onSubmit={(e) => e.preventDefault()}>
            <input
              ...
              className="flex-1 min-w-0 rounded-full bg-white/10 border border-white/20 px-4 py-2 text-sm placeholder:text-white/50 focus:outline-none focus:border-white/40"
            />
            <button className="btn-gold rounded-full px-4 py-2 text-sm font-semibold text-black">
              Subscribe
            </button>
          </form>
          <a
...
function SocialIcon({
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="w-9 h-9 rounded-full glass-dark grid place-items-center hover:bg-white/20 transition-colors"
    >
      <Icon size={16} />
    </a>
  );
